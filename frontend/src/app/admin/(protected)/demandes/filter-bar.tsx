...
import { PROPERTY_LABEL, TXN_LABEL, PRIORITIES } from './labels';
 ...
 * `to` is pushed to end-of-day so "au J" still includes requests filed on
...
export function filtersToQuery(f: Filters): string {
  ...
  add('minBudget', f.minBudget);
  add('maxBudget', f.maxBudget);
  add('from', f.from);
...
function budgetSummary(f: Filters): string {
  if (f.minBudget && f.maxBudget) return `${fmtInt(f.minBudget)} – ${fmtInt(f.maxBudget)}`;
  if (f.minBudget) return `≥ ${fmtInt(f.minBudget)}`;
  if (f.maxBudget) return `≤ ${fmtInt(f.maxBudget)}`;
  return 'Budget';
}
...
export function DemandesFilterBar({
  value,
  onChange,
}: {
  value: Filters;
  ...
  const [open, setOpen] = useState<null | 'budget' | 'date'>(null);
  const barRef = useRef<HTMLDivElement>(null);

  // Text inputs (search, budget, dates) propagate on a short debounce so
  // typing doesn't fire a request per keystroke; the selects propagate
  // immediately.
  ...
  }, [value.q, value.minBudget, value.maxBudget, value.from, value.to]);

  useEffect(() => {
    const same =
      draft.q === value.q &&
      draft.minBudget === value.minBudget &&
      draft.maxBudget === value.maxBudget &&
  ...
  const budgetActive = value.minBudget !== '' || value.maxBudget !== '';
    ...
    >
      <label className="flex h-10 min-w-[220px] flex-1 items-center gap-2 rounded-[10px] border border-black/[0.08] px-3 text-[13px]">
        ...
        <input
          ...
          placeholder="Rechercher par nom du demandeur, ville…"
      ...
      <PopoverPill
        icon={Coins}
        label={budgetActive ? budgetSummary(value) : 'Budget'}
        active={budgetActive}
        isOpen={open === 'budget'}
        onToggle={() => setOpen(open === 'budget' ? null : 'budget')}
      >
        <p className="mb-2 text-[12px] font-semibold text-neutral-900">Budget (FCFA)</p>
        ...
        {budgetActive && (
          <button
            type="button"
            onClick={() => setDraft((d) => ({ ...d, minBudget: '', maxBudget: '' }))}
      ...
      <PopoverPill
        icon={CalendarDays}
        label={dateActive ? dateSummary(value) : 'Date de dépôt'}
        active={dateActive}
        isOpen={open === 'date'}
        onToggle={() => setOpen(open === 'date' ? null : 'date')}
      >
        <p className="mb-2 text-[12px] font-semibold text-neutral-900">Date de dépôt</p>
